...
async function getPublishableKey({ projectId }: { projectId: string }) {
  ...
  const publishableKey = keys.find(
    (key: any) => key["name"] === "anon" || key["type"] === "publishable",
  );

  if (!publishableKey) {
    throw new Error(
      "No publishable key found for project. Make sure you are connected to the correct Supabase account and project. See https://codeduet.com/docs/integrations/supabase#no-publishable-keys",
    );
  }
