...
export function SetupBanner() {
  ...
  // Watch for provider setup completion and trigger confetti
  useEffect(() => {
    if (!loading) {
  ...
  const handleOpenAISetupClick = () => {
    posthog.capture("setup-flow:ai-provider-setup:openai:click");
    navigate({
      to: providerSettingsRoute.id,
  ...
  const handleRunpodSetupClick = () => {
    posthog.capture("setup-flow:ai-provider-setup:runpod:click");
    navigate({
      to: providerSettingsRoute.id,
  ...
  const handleAnthropicSetupClick = () => {
    posthog.capture("setup-flow:ai-provider-setup:anthropic:click");
    navigate({
      to: providerSettingsRoute.id,
        ...
        >
          ...
          >
            ...
            <AccordionContent className="px-4 pt-2 pb-4 bg-white dark:bg-zinc-900 border-t border-inherit">
              ...
              <SetupProviderCard
                variant="openai"
                onClick={handleOpenAISetupClick}
                tabIndex={isNodeSetupComplete ? 0 : -1}
                leadingIcon={
                  <Sparkles className="w-4 h-4 text-teal-600 dark:text-teal-400" />
                }
                title="Setup ChatGPT API Key"
                subtitle="OpenAI's most advanced AI model"
              />
              ...
              <SetupProviderCard
                ...
                subtitle="Google's advanced AI models"
              />

              <SetupProviderCard
                className="mt-2"
                variant="runpod"
                onClick={handleRunpodSetupClick}
                tabIndex={isNodeSetupComplete ? 0 : -1}
                leadingIcon={
                  <Sparkles className="w-4 h-4 text-green-600 dark:text-green-400" />
                }
                title="Setup Runpod API Key"
                subtitle="Serverless GPU inference endpoints"
              />

              <SetupProviderCard
                className="mt-2"
                variant="anthropic"
                onClick={handleAnthropicSetupClick}
                tabIndex={isNodeSetupComplete ? 0 : -1}
                leadingIcon={
                  <Sparkles className="w-4 h-4 text-orange-600 dark:text-orange-400" />
                }
                title="Setup Anthropic API Key"
                subtitle="Claude's powerful AI models"
              />
    ...
    <div className="mt-3 p-3 bg-(--background-lighter) border rounded-lg text-sm">
      <p>
        ...
        <a
          onClick={() => {
            IpcClient.getInstance().openExternalUrl(
              "https://www.codeduet.com/docs/help/nodejs",
            );
          }}
...
export const OpenRouterSetupBanner = ({
  ...
  return (
    <SetupProviderCard
      ...
      subtitle="Access to multiple AI models"
    />
  );
};
