...
export function ModelPicker() {
  ...
  // Split providers into primary, secondary, and CLI groups (excluding auto)
  ...
  const primaryProviders = providerEntries.filter(([providerId, models]) => {
    ...
    return !(provider && provider.secondary) && provider?.type !== "cli";
  });
  ...
  const cliProviders = providerEntries.filter(([providerId, models]) => {
    if (models.length === 0) return false;
    const provider = providers?.find((p) => p.id === providerId);
    ...
    <DropdownMenu open={open} onOpenChange={setOpen}>
      ...
      >
        ...
        {cliProviders.length > 0 && (
          <>
            <DropdownMenuSeparator />
            <DropdownMenuLabel>CLI Providers</DropdownMenuLabel>
            <DropdownMenuSeparator />
            {cliProviders.map(([providerId, models]) => {
              const provider = providers?.find((p) => p.id === providerId);
              return (
                <DropdownMenuSub key={providerId}>
                  <DropdownMenuSubTrigger className="w-full font-normal">
                    <div className="flex flex-col items-start w-full">
                      <div className="flex items-center gap-2">
                        <span>{provider?.name ?? providerId}</span>
                        <span className="text-[10px] bg-blue-500/20 text-blue-700 px-1.5 py-0.5 rounded-full font-medium">
                          CLI
                        </span>
                      </div>
                      <span className="text-xs text-muted-foreground">
