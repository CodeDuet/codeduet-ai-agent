...
export function registerCheckpointHandlers() {
  handle(
    ...
    async (_, messageId: number, chatId: number) => {
  ...
  handle(
    "checkpoint:restore",
    async (_, checkpointHash: string, chatId: number) => {
  ...
  handle(
    "checkpoint:cleanup",
    async (_, chatId: number, keepLatest: number = 10) => {
      logger.log(
        `Cleaning up old checkpoints for chat ${chatId}, keeping latest ${keepLatest}`,
      );
