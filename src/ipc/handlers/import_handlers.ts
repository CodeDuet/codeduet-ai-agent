...
export function registerImportHandlers() {
  ...
  handle("check-app-name", async (_, { appName }: { appName: string }) => {
    // Since we now use the original directory path, we only need to check the database
  ...
  handle(
    ...
    async (
      ...
      // Check if the app name already exists in the database
      const existingApp = await db.query.apps.findFirst({
        where: eq(apps.name, appName),
      });
      ...
      // Use the original source path directly instead of copying
      ...
      if (!isGitRepo) {
        ...
        await gitCommit({
          path: projectPath,
          message: "Init CodeDuet app",
        });
      }

      // Create a new app with the original path
      const [app] = await db
        .insert(apps)
        .values({
          name: appName,
          // Store the full original path instead of just the name
          path: projectPath,
