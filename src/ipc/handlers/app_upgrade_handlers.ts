...
import { getCodeDuetAppPath, resolveAppPath } from "../../paths/paths";
...
const availableUpgrades: Omit<AppUpgrade, "isNeeded">[] = [
  {
    ...
    manualUpgradeUrl: "https://codeduet.com/docs/upgrades/select-component",
  },
  {
    id: "capacitor",
    ...
    manualUpgradeUrl:
      "https://codeduet.com/docs/guides/mobile-app#upgrade-your-app",
  },
];
...
function isComponentTaggerUpgradeNeeded(appPath: string): boolean {
  ...
  try {
    ...
    return !viteConfigContent.includes(
      "@codeduet-sh/react-vite-component-tagger",
    );
  } catch (e) {
...
async function applyComponentTagger(appPath: string) {
  ...
  if (
    !content.includes(
      "import dyadComponentTagger from '@codeduet-sh/react-vite-component-tagger';",
    )
  ) {
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    const process = spawn(
      "pnpm add -D @codeduet-sh/react-vite-component-tagger || npm install --save-dev --legacy-peer-deps @codeduet-sh/react-vite-component-tagger",
      {
        cwd: appPath,
        shell: true,
        stdio: "pipe",
...
export function registerAppUpgradeHandlers() {
  handle(
    ...
    async (_, { appId }: { appId: number }): Promise<AppUpgrade[]> => {
      ...
      const appPath = resolveAppPath(app.path);
