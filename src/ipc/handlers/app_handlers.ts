      ...
      if (urlMatch) {
        proxyWorker = await startProxy(urlMatch[1], {
          onStarted: (proxyUrl) => {
            safeSend(event.sender, "app:output", {
              type: "stdout",
              message: `[codeduet-proxy-server]started=[${proxyUrl}] original=[${urlMatch[1]}]`,
              appId,
            });
          },
        });
...
export function registerAppHandlers() {
  ...
  handle(
    ...
    async (
      ...
      const fullAppPath = getCodeDuetAppPath(appPath);
  ...
  handle(
    ...
    async (_, params: CopyAppParams): Promise<{ app: any }> => {
      ...
      const originalAppPath = getCodeDuetAppPath(originalApp.path);
      const newAppPath = getCodeDuetAppPath(newAppName);
  ...
  ipcMain.handle("list-apps", async () => {
    ...
    return {
      apps: allApps,
      appBasePath: getCodeDuetAppPath("$APP_BASE_PATH"),
    };
  });
  ...
  ipcMain.handle(
    ...
    async (
      ...
      return withLock(appId, async () => {
        ...
        const oldAppPath = resolveAppPath(app.path);
        const newAppPath = getCodeDuetAppPath(appPath);
  ...
  ipcMain.handle("reset-all", async (): Promise<void> => {
    ...
    const dyadAppPath = getCodeDuetAppPath(".");
