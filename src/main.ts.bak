...
import { initializeMcpServers } from "./ipc/handlers/mcp_handlers";
...
if (process.defaultApp) {
  if (process.argv.length >= 2) {
    app.setAsDefaultProtocolClient("codeduet", process.execPath, [
      path.resolve(process.argv[1]),
    ]);
  }
} else {
  app.setAsDefaultProtocolClient("codeduet");
}
...
export async function onReady() {
  ...
  if (settings.enableAutoUpdate) {
    ...
    const host = `https://api.codeduet.com/v1/update/${postfix}`;
    logger.info("Auto-update release channel=", postfix);
    updateElectronApp({
      logger,
      updateSource: {
        type: UpdateSourceType.ElectronPublicUpdateService,
        repo: "codeduet/codeduet-ai-agent",
        host,
      },
...
function handleDeepLinkReturn(url: string) {
  // example url: "codeduet://supabase-oauth-return?token=a&refreshToken=b"
  let parsed: URL;
  try {
    parsed = new URL(url);
  } catch {
  ...
  if (parsed.protocol !== "codeduet:") {
    dialog.showErrorBox(
      "Invalid Protocol",
      `Expected codeduet://, got ${parsed.protocol}. Full URL: ${url}`,
    );
    return;
  }
  ...
  // codeduet://codeduet-pro-return?key=123&budget_reset_at=2025-05-26T16:31:13.492000Z&max_budget=100
